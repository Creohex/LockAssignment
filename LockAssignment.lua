...
function LockAssignmentFrame_OnUpdate(self, elapsed)
    if (self.TimeSinceLastClockUpdate == nil) then
        self.TimeSinceLastClockUpdate = 0;
    end
    if (self.TimeSinceLastSSCDUpdate == nil) then
        self.TimeSinceLastSSCDUpdate = 0;
    end
    if (self.TimeSinceLastSSCDBroadcast == nil) then
...
function LA.Reset_SS()
    LA.ClearSSAssignments()
    LA.UpdateAllWarlockFrames();
end
