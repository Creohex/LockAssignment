...
function LA.InitLockAssignmentFrameScrollArea() -- parent frame
    ...
    LockAssignmentFrame.WarningTextFrame.value = LA.AddTextToFrame(LockAssignmentFrame.WarningTextFrame,
        "Warning your addon is out of date!", 240)
...
function LA.CreateSSCooldownTracker(ParentFrame)
    local TextFrame = LA.AddTextToFrame(ParentFrame, "Unknown", 140)
...
function LA.CreateSSAssignmentMenu(ParentFrame)
    ...
    local SetNoneButton = CreateFrame("Button", nil, ParentFrame, "UIPanelButtonTemplate");
    SetNoneButton:SetWidth(24);
    SetNoneButton:SetHeight(24);
    SetNoneButton:SetPoint("RIGHT", SSAssignmentMenu, "LEFT", 15, 2)
    SetNoneButton:SetText("x");
    SetNoneButton:SetScript("OnClick", function()
		LA.ClearSSAssignments(ParentFrame.WarlockName)
...
function LA.InitLockAssignmentCheckFrame()
    ...
    LockAssignmentAssignCheckFrame.Label = LA.AddTextToFrame(LockAssignmentAssignCheckFrame, "Your new assignments:",
        140)
    ...
    LockAssignmentAssignCheckFrame.SoulStoneAssignment:SetPoint("TOPLEFT", LockAssignmentAssignCheckFrame, "TOPLEFT",
        65, -97)
...
function LA.InitPersonalMonitorFrame()
    ...
    AssignmentPersonalMonitorFrame.BanishGraphicFrame:SetPoint("LEFT", AssignmentPersonalMonitorFrame.CurseGraphicFrame,
        "RIGHT", 2, 0)

    AssignmentPersonalMonitorFrame.SSAssignmentText = LA.AddTextToFrame(AssignmentPersonalMonitorFrame, "", 75);
    AssignmentPersonalMonitorFrame.SSAssignmentText:SetPoint("LEFT", AssignmentPersonalMonitorFrame.BanishGraphicFrame,
        "RIGHT", 5, 0)
...
function LA.UpdatePersonalSSAssignment(ParentFrame, SSAssignment)
    if SSAssignment ~= "None" then
        ParentFrame.SSAssignmentText:SetText("SS: " .. SSAssignment);
    else
...
function LA.UpdatePersonalMonitorFrame()
    ...
    if myData.CurseAssignment == "None" and myData.BanishAssignment ~= "None" then
        ...
        AssignmentPersonalMonitorFrame.SSAssignmentText:SetPoint("LEFT",
            AssignmentPersonalMonitorFrame.BanishGraphicFrame, "RIGHT", 5, 0)
    end
    if myData.BanishAssignment == "None" and myData.CurseAssignment ~= "None" then
        ...
        AssignmentPersonalMonitorFrame.SSAssignmentText:SetPoint("LEFT",
            AssignmentPersonalMonitorFrame.CurseGraphicFrame, "RIGHT", 5, 0)
    end
...
function LA.SetExtraChats()
    LA.AnnouncerOptions = {"Addon Only", "Raid", "Party", "Whisper", "Class Channel"}
