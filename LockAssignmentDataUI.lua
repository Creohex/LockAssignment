...
function LA.UpdateAssignmentFrame(Warlock, AssignmentFrame)
    ...
    UIDropDownMenu_SetSelectedID(AssignmentFrame.CurseAssignmentMenu,
        LA.GetIndexFromTable(LA.CurseOptions, Warlock.CurseAssignment))
    LA.UpdateCurseGraphic(AssignmentFrame.CurseAssignmentMenu,
        LA.GetCurseValueFromDropDownList(AssignmentFrame.CurseAssignmentMenu))
    UIDropDownMenu_SetText(LA.GetCurseValueFromDropDownList(AssignmentFrame.CurseAssignmentMenu),
        AssignmentFrame.CurseAssignmentMenu)

    -- Set the BanishAssignmentMenu
    UIDropDownMenu_SetSelectedID(AssignmentFrame.BanishAssignmentMenu,
        LA.GetIndexFromTable(LA.BanishMarkers, Warlock.BanishAssignment))
    LA.UpdateBanishGraphic(AssignmentFrame.BanishAssignmentMenu,
        LA.GetValueFromDropDownList(AssignmentFrame.BanishAssignmentMenu, LA.BanishMarkers, ""))
    UIDropDownMenu_SetText(LA.GetValueFromDropDownList(AssignmentFrame.BanishAssignmentMenu, LA.BanishMarkers, ""),
        AssignmentFrame.BanishAssignmentMenu)
    ...
    UIDropDownMenu_SetSelectedID(AssignmentFrame.SSAssignmentMenu,
        LA.GetSSIndexFromTable(LA.GetSSTargets(), Warlock.SSAssignment))
    ...
    if selecedSSTarget ~= nil then
        ...
        UIDropDownMenu_SetText(LA.GetValueFromDropDownList(AssignmentFrame.SSAssignmentMenu, LA.GetSSTargets(),
            "SSAssignments"), AssignmentFrame.SSAssignmentMenu)
    end
...
function LA.UpdateLockClock(warlock)
    ...
    if (warlock.SSonCD == "true" and warlock.SSCooldown ~= nil) then
        ...
        if (LA.DebugMode) then
            LA.print(warlock.Name, "my time:", warlock.MyTime, "localtime:", warlock.LocalTime, "timeShift:", timeShift,
                "LocalCD", warlock.SSCooldown, "Abs CD:", absCD, "Time Remaining:", secondsRemaining)
        end
...
function LA.CheckSSCD(self)
    ...
    if myself ~= nil then
        ...
        if (startTime ~= nil and startTime > 0 and self.TimeSinceLastSSCDBroadcast >
            LA.LockAssignmentSSCD_BroadcastInterval) then
...
function LA.GetWarlockFromAssignmentFrame(WarlockName)
    ...
    local Warlock = LA.CreateWarlock(AssignmentFrame.WarlockName,
        LA.GetCurseValueFromDropDownList(AssignmentFrame.CurseAssignmentMenu), LA.GetValueFromDropDownList(
            AssignmentFrame.BanishAssignmentMenu, LA.BanishMarkers, ""))
    Warlock.SSAssignment = LA.GetValueFromDropDownList(AssignmentFrame.SSAssignmentMenu, LA.GetSSTargets(),
        "SSAssignments")
...
function LA.IsUIDirty(AssignmentData)
    ...
    for k, v in pairs(AssignmentData) do
        ...
        if (v.CurseAssignment ~= uiLock.CurseAssignment or v.BanishAssignment ~= uiLock.BanishAssignment or
...
-- Clears soulstone assignments
function LA.ClearSSAssignments(name)
	if name then
		for k, v in pairs(LA.LockAssignmentsData) do
			if v.Name == name then
				local uiLock = LA.GetWarlockFromAssignmentFrame(v.Name)
				if LA.DebugMode then
					LA.print("Clearing", v.SSAssignment)
				end
				uiLock.SSAssignment = "None"
				v.SSAssignment = "None"
			end
		end
	else
...
function LA.SendAnnounceMent(AnnounceOption, message, v)
    ...
    elseif AnnounceOption == "Class Channel" then
        SendChatMessage(message, "CHANNEL", nil, GetChannelName("scourgewarlocks"))
    else
        if (LA.DebugMode) then
